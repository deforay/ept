				...
				if(isset($arrayVal[$schemeKey])){
					...
					if($certificate && $participated){
						...
						if($schemeKey == 'eid'){
							...
							//$doc->save("certificate/2017 Certificate - ".strtoupper($schemeKey)." for Lab ".str_replace('/', '_', $key).".docx");					
							$doc->save("certificate/excellence/".str_replace('/', '_', $key)."-EID.docx");
							...
							$doc->setValue("DATE","24 January 2017");
							//$doc->save("certificate/2017 Certificate - ".strtoupper($schemeKey)." for Lab ".str_replace('/', '_', $key).".docx");	
							$doc->save("certificate/excellence/".str_replace('/', '_', $key)."-VL.docx");
						}
						...
						if($schemeKey == 'eid'){
							...
							$doc->setValue("LABNAME",$arrayVal['labName']);
							$doc->setValue("DATE","24 January 2017");
							//$doc->save("certificate/2017 Certificate - ".strtoupper($schemeKey)." for Lab ".str_replace('/', '-', $key).".docx");	
							$doc->save("certificate/participation/".str_replace('/', '_', $key)."-EID.docx");
						}else if($schemeKey=='vl'){
							if($attribs["vl_assay"]==6){
								if(isset($attribs["other_assay"])){
									$assay=$attribs["other_assay"];	
							...
							$doc->setValue("DATE","24 January 2017");
							//$doc->save("certificate/2017 Certificate - ".strtoupper($schemeKey)." for Lab ".str_replace('/', '-', $key).".docx");
							$doc->save("certificate/participation/".str_replace('/', '_', $key)."-VL.docx");
						}
					}
