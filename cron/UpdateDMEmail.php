...
try {
    ...
    $sQuery = $db->select()->from(array('dm' => 'data_manager'))
            ->join(array('p' => 'participant'), 'p.email=dm.primary_email',array('p.participant_id','unique_identifier'=>new Zend_Db_Expr('MAX(p.unique_identifier)')))
            ->group("dm.dm_id");
