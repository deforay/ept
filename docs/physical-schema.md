...
This is a column inventory, not executable DDL. It omits full index definitions,
constraint names, collations, triggers, routines and events.
A schema-only export remains necessary for a complete physical schema handover.

Source: read-only queries of `information_schema.COLUMNS`, `TABLES` and `KEY_COLUMN_USAGE`.
The version comes from `system_config.app_version`.
Business meanings appear in the [core dictionary](data-model.md).

Installations can differ because of migration history or local extensions.
Matching version labels alone does not establish identical schemas.

## Notation

`PRI`, `UNI` and `MUL` reproduce MySQL column-key metadata. They do not describe complete composite indexes.
`NULL` in Default means the metadata reports no non-null default.
`(empty string)` is an explicit empty-string default. Blank Key or Extra means no reported value.
Foreign-key entries preserve the metadata rows, including repeated column relationships.
...
| Column | SQL type | Nullable | Default | Key | Extra |
| --- | --- | --- | --- | --- | --- |
...
| Foreign-key column | Referenced table | Referenced column |
| --- | --- | --- |
