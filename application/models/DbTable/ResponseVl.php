...
class Application_Model_DbTable_ResponseVl extends Zend_Db_Table_Abstract
{
    ...
    public function updateResultsByAPI($params,$dm){
        $sampleIds = $params["vlData"]->Heading3->data->sampleId;
        foreach($sampleIds as $key => $sampleId){
            $res = $this->fetchRow("shipment_map_id = ".$params['mapId'] . " and sample_id = '".$sampleId ."'");
            //Set tnd value if Yes
            $tnd = NULL;
            if(isset($params["vlData"]->Heading3->data->ptPanelTest) && $params["vlData"]->Heading3->data->ptPanelTest== 'yes'){
                $params["vlData"]->Heading3->data->vlResult[$key] = '';
            }else if(isset($params["vlData"]->Heading3->data->isTND[$key]) && $params["vlData"]->Heading3->data->isTND[$key]== 'yes'){
                $tnd = 'yes';
                $params["vlData"]->Heading3->data->vlResult[$key] = '0.00'; 
            }
            ...
            if($res == null || $res === false){
                $this->insert(array(
                    ...
                    'sample_id'             =>  $sampleId,
                    'reported_viral_load'   =>  $params["vlData"]->Heading3->data->vlResult[$key],
                    'is_tnd'                =>  $tnd,
                    'created_by'            =>  $dm['dm_id'],
