...
class Application_Model_DbTable_DataManagers extends Zend_Db_Table_Abstract
{
    ...
    public function getAllUsers($parameters)
    {
        ...
        if (isset($parameters['sSearch']) && $parameters['sSearch'] != '') {
            ...
            foreach ($searchArray as $search) {
                ...
                for ($i = 0; $i < $colSize; $i++) {
                    if ($i < $colSize - 1) {
                        $sWhereSub .= $searchableColumns[$i] . ' LIKE ' . Pt_Commons_General::sqlLikeContains($search) . ' OR ';
                    } else {
                        $sWhereSub .= $searchableColumns[$i] . ' LIKE ' . Pt_Commons_General::sqlLikeContains($search) . ' ';
                    }
                }
                $sWhereSub .= ')';
            }
        ...
        if (!empty($pageDmIds)) {
            $countSelect = $this->getAdapter()->select()
                ->from('participant_manager_map', ['dm_id', 'cnt' => new Zend_Db_Expr('COUNT(*)')])
    ...
    public function resetpasswordForEmail($email)
    {
        ...
        $sql = $this->select()->from('data_manager')
    ...
    public function updatePassword($oldpassword, $newpassword)
    {
        ...
        if ($passwordVerify) {
            $newpassword = Common::passwordHash($newpassword);
            $noOfRows = $this->update(
                ['password' => $newpassword, 'force_password_reset' => 0],
                ['primary_email = ?' => $email]
            );
            if ($noOfRows != null && $noOfRows == 1) {
    ...
    public function fetchParticipantDatamanagerSearch($searchParams)
    {
        ...
        if (isset($searchParams) && !empty($searchParams)) {
            $sql = $sql->where('primary_email LIKE ? OR first_name LIKE ? OR last_name LIKE ? OR institute LIKE ?', '%' . $searchParams . '%');
        }
        $authNameSpace = new Zend_Session_Namespace('datamanagers');
    ...
    public function loginDatamanagerByAPI($params)
    {
        ...
        $result = $this->fetchRow(
            $this->select()
                ->where('new_email != primary_email')
                ->where('new_email = ?', $params['userId'])
        );
        ...
        $result = $this->fetchDataManagerByEmail($params['userId']);
        $passwordVerify = true;
        if (isset($result) && !empty($result)) {
            $passwordVerify = password_verify((string) $params['key'], (string) $result['password']);
        }
    ...
    public function changePasswordDatamanagerByAPI($params)
    {
        ...
        $oldPassResult = $this->fetchDataManagerByAuthToken($params['authToken']);
        $passwordVerify = true;
        ...
        if (!$oldPassResult && !$passwordVerify) {
            return ['status' => 'fail', 'message' => 'Your old password is incorrect', 'profileInfo' => $aResult['profileInfo']];
        }
    ...
    public function setForgetPasswordDatamanagerAPI($params)
    {
        ...
        /* App version check */
        $aResult = $this->fetchDataManagerByEmail($params['email']);
        if (!$aResult) {
        ...
        $email = $params['email'];
        $row = $this->fetchDataManagerByEmail($email);
    ...
    {
        return $this->fetchDataManagerByAuthToken($params['authToken']);
    }
                ...
                if ($locationWiseSwitch) { // Check the status activated or not
                    ...
                    if ($db->fetchRow($ptccQuery)) {
                        if (!empty($params['participantsList'])) {
                            $db->delete('participant_manager_map', ['participant_id NOT IN (?)' => $params['participantsList'], 'dm_id LIKE ?' => $dmId]);
                        }
    ...
    {
        // Authentication should only look up by identity; the password check happens via password_verify().
        return $this->fetchDataManagerByEmail($email);
    }

    private function fetchDataManagerByEmail($email)
    {
    ...
    private function fetchDataManagerByAuthToken($authToken)
    {
        return $this->fetchRow(
            $this->select()
                ->from('data_manager')
