...
class Application_Model_DbTable_Shipments extends Zend_Db_Table_Abstract
{
    ...
    {
        $result = $this->getAdapter()->fetchRow($this->getAdapter()->select()->from(['s' => 'shipment'])
            ->joinLeft(['d' => 'distributions'], 'd.distribution_id = s.distribution_id', ['distribution_code', 'distribution_date'])
            ->joinLeft(['sp' => 'shipment_participant_map'], 's.shipment_id=sp.shipment_id', ['map_id', 'number_of_tests'])
            ->joinLeft(['p' => 'participant'], 'sp.participant_id=p.participant_id', ['participant_id', 'unique_identifier', 'institute_name', 'anc'])
            ->joinLeft(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['sl.scheme_name'])
            ->group('s.shipment_id')
            ->where('s.shipment_id = ?', $sId));
        if ($result != '') {
            ...
            $result['referenceResult'] = $this->getAdapter()->fetchAll($this->getAdapter()->select()->from([$tableName])
    ...
    public function getShipmentOverviewDetails($parameters)
    {
        ...
        if (!empty($authNameSpace->dm_id)) {
            $sQuery = $sQuery->join(['p' => 'participant'], 'p.participant_id=sp.participant_id', ['p.unique_identifier', 'p.first_name', 'p.last_name', 'p.participant_id']);
            $sQuery = $sQuery
    ...
    public function getShipmentCurrentDetails($parameters)
    {
        ...
        $aColumns = ['DATE_FORMAT(shipment_date,"%d-%b-%Y")', 'scheme_name', 'shipment_code', 'distribution_code', 'unique_identifier', new Zend_Db_Expr(Application_Model_DbTable_Participants::participantNameExpr('p')), 'p.institute_name', 'DATE_FORMAT(lastdate_response,"%d-%b-%Y")', 'DATE_FORMAT(spm.shipment_test_report_date,"%d-%b-%Y")'];
        $orderColumns = ['shipment_date', 'scheme_name', 'shipment_code', 'distribution_code', 'unique_identifier', 'first_name', 'p.institute_name', 'lastdate_response', 'spm.shipment_test_report_date'];
        ...
        $sQuery = $this->getAdapter()->select()
            ->from(['s' => 'shipment'], [new Zend_Db_Expr('SQL_CALC_FOUND_ROWS s.scheme_type'), 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.status', 's.response_switch', 'allow_editing_response', 'panelName' => new Zend_Db_Expr('shipment_attributes->>"$.panelName"')])
    ...
    public function getDefaultedShipments($parameters)
    {
        ...
        $aColumns = ['year(shipment_date)', 'DATE_FORMAT(shipment_date,"%d-%b-%Y")', 'scheme_name', 'shipment_code', 'unique_identifier', 'first_name', 'DATE_FORMAT(lastdate_response,"%d-%b-%Y")', 'DATE_FORMAT(spm.shipment_test_report_date,"%d-%b-%Y")'];
        $orderColumns = ['shipment_date', 'shipment_date', 'scheme_name', 'shipment_code', 'unique_identifier', 'first_name', 'lastdate_response', 'spm.shipment_test_report_date'];
        ...
        $sQuery = $this->getAdapter()->select()
            ->from(['s' => 'shipment'], [new Zend_Db_Expr('SQL_CALC_FOUND_ROWS s.scheme_type'), 's.status', 'SHIP_YEAR' => 'year(s.shipment_date)', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.response_switch'])
            ->join(['spm' => 'shipment_participant_map'], 'spm.shipment_id=s.shipment_id', ['spm.map_id', 'spm.evaluation_status', 'spm.participant_id', 'RESPONSEDATE' => "DATE_FORMAT(spm.shipment_test_report_date,'%Y-%m-%d')", 'ACTION' => new Zend_Db_Expr("CASE  WHEN substr(spm.evaluation_status,2,1)='1' THEN 'View' WHEN (substr(spm.evaluation_status,2,1)='9' AND s.lastdate_response>= CURDATE()) OR (s.status= 'finalized') THEN 'Enter Result' END"), 'STATUS' => new Zend_Db_Expr("CASE substr(spm.evaluation_status,3,1) WHEN 1 THEN 'On Time' WHEN '2' THEN 'Late' WHEN '0' THEN 'No Response' END")])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['scheme_name', 'is_user_configured'])
            ->join(['p' => 'participant'], 'p.participant_id=spm.participant_id', ['p.unique_identifier', 'p.first_name', 'p.last_name', 'p.participant_id', 'p.country'])
            ->joinLeft(['c' => 'countries'], 'p.country=c.id', ['c.iso_name'])
    ...
    public function getShipmentAllDetails($parameters)
    {
        ...
        $sQuery = $this->getAdapter()->select()
            ->from(['s' => 'shipment'], [new Zend_Db_Expr('SQL_CALC_FOUND_ROWS s.scheme_type'), 'SHIP_YEAR' => 'year(s.shipment_date)', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.status', 's.response_switch'])
            ->join(['spm' => 'shipment_participant_map'], 'spm.shipment_id=s.shipment_id', ['spm.report_generated', 'spm.map_id', 'spm.evaluation_status', 'qc_date', 'spm.participant_id', 'RESPONSEDATE' => "DATE_FORMAT(spm.shipment_test_report_date,'%Y-%m-%d')", 'RESPONSE' => new Zend_Db_Expr("CASE  WHEN substr(spm.evaluation_status,3,1)='1' THEN 'View' WHEN (substr(spm.evaluation_status,3,1)='9' AND s.lastdate_response >= CURDATE()) OR (substr(spm.evaluation_status,3,1)='9' AND s.status= 'finalized') THEN 'Enter Result' END"), 'REPORT' => new Zend_Db_Expr("CASE  WHEN spm.report_generated='yes' AND s.status='finalized' THEN 'Report' END")])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['scheme_name', 'is_user_configured'])
    ...
    public function getindividualReportDetails($parameters)
    {
        ...
        $sQuery = $this->getAdapter()->select()->from(['s' => 'shipment'], [new Zend_Db_Expr('SQL_CALC_FOUND_ROWS s.scheme_type'), 'SHIP_YEAR' => 'year(s.shipment_date)', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.corrective_action_file', 'shipmentStatus' => 's.status', 'collect_feedback', 'feedback_expiry_date'])
    ...
    public function fetchCorrectiveActionReport($parameters)
    {
        ...
        $sQuery = $this->getAdapter()->select()->from(['s' => 'shipment'], [new Zend_Db_Expr('SQL_CALC_FOUND_ROWS s.scheme_type'), 'SHIP_YEAR' => 'year(s.shipment_date)', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.corrective_action_file'])
            ->join(['sl' => 'scheme_list'], 's.scheme_type=sl.scheme_id', ['scheme_name'])
            ->join(['spm' => 'shipment_participant_map'], 'spm.shipment_id=s.shipment_id', ['spm.map_id', 'final_result', 'spm.evaluation_status', 'spm.participant_id', 'RESPONSEDATE' => "DATE_FORMAT(spm.shipment_test_report_date,'%Y-%m-%d')", 'RESPONSE' => new Zend_Db_Expr("CASE WHEN ASCII(SUBSTRING(spm.evaluation_status FROM 3 FOR 1)) = ASCII('1') THEN 'View' WHEN ASCII(SUBSTRING(spm.evaluation_status FROM 3 FOR 1)) = ASCII('9') THEN 'Enter Result' END"), 'REPORT' => new Zend_Db_Expr("CASE  WHEN spm.report_generated='yes' AND s.status='finalized' THEN 'Report' END")])
            ->join(['p' => 'participant'], 'p.participant_id=spm.participant_id', ['p.unique_identifier', 'p.first_name', 'p.last_name'])
            // ->join(array('pmm' => 'participant_manager_map'), 'pmm.participant_id=p.participant_id')
            // ->where("pmm.dm_id=?", $this->_session->dm_id)
            ->where("s.status='finalized'")
    ...
    public function getAllShipmentFormDetails($parameters)
    {
        ...
        $aColumns = ['sl.scheme_name', 'shipment_code', 'distribution_code', "DATE_FORMAT(distribution_date,'%d-%b-%Y')", "DATE_FORMAT(lastdate_response,'%d-%b-%Y')"];
        $orderColumns = ['sl.scheme_name', 'shipment_code', 'distribution_code', 'distribution_date', 'lastdate_response'];
        ...
        $sQuery = $db->select()->from(['s' => 'shipment'], [new Zend_Db_Expr('SQL_CALC_FOUND_ROWS *')])
            ->join(['d' => 'distributions'], 'd.distribution_id = s.distribution_id', ['distribution_code', 'distribution_date'])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['SCHEME' => 'sl.scheme_name', 'scheme_id'])
    ...
    public function fetchParticipantSchemesBySchemeId($parameters)
    {
        ...
        $sQuery = $this->getAdapter()->select()->from(['s' => 'shipment'], [new Zend_Db_Expr('SQL_CALC_FOUND_ROWS s.scheme_type'), 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.status', 's.response_switch'])
            ->join(['spm' => 'shipment_participant_map'], 'spm.shipment_id=s.shipment_id', ['spm.report_generated', 'spm.map_id', 'spm.evaluation_status', 'qc_date', 'spm.participant_id', 'RESPONSEDATE' => "DATE_FORMAT(spm.shipment_test_report_date,'%Y-%m-%d')", 'spm.shipment_score'])
    ...
    public function fetchUniqueShipmentCode()
    {
        return $this->getAdapter()->fetchAll($this->getAdapter()
            ->select()->from(['s' => $this->_name], ['shipment_code' => new Zend_Db_Expr(' DISTINCT s.shipment_code '), 'shipment_id', 'response_switch'])
    ...
    public function fetchShipmentDetailsInAPI($params, $type)
    {
        ...
        $sQuery = $this->getAdapter()->select()->from(['s' => 'shipment'], ['s.scheme_type', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.status', 's.response_switch', 's.updated_on_admin'])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['scheme_name'])
        ...
        $rResult = $this->getAdapter()->fetchAll($sQuery);
        if (empty($rResult)) {
            return ['status' => 'fail', 'message' => 'Shipment Details not available'];
        }
        ...
        if ($type == 'form') {
            if ($checkFormSatatus) {
                return ['status' => 'success', 'data' => $formData, 'profileInfo' => $aResult['profileInfo']];
            } else {
                return ['status' => 'fail', 'message' => "The following shipments doesn't have the shipment forms", 'data' => $getParticipantDetails, 'profileInfo' => $aResult['profileInfo']];
            }
        }
        return ['status' => 'success', 'data' => $data, 'profileInfo' => $aResult['profileInfo']];
    }
    ...
    public function fetchSchemeTypeShipmentDetailsInAPIV2($params)
    {
        ...
        /* Check the app versions & parameters */
        if (!isset($params['appVersion'])) {
            return ['status' => 'version-failed', 'message' => 'App version is not updated. Kindly go to the play store and update the app'];
        }
        if (!isset($params['authToken'])) {
            return ['status' => 'auth-fail', 'message' => 'Something went wrong. Please log in again'];
        }
        ...
        /* To check the shipment details for the data managers mapped participants */
        $sQuery = $this->getAdapter()->select()->from(['s' => 'shipment'], ['s.scheme_type', 's.shipment_attributes', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.status', 's.response_switch', 's.updated_on_admin'])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['scheme_name', 'is_user_configured'])
            ->join(['spm' => 'shipment_participant_map'], 'spm.shipment_id=s.shipment_id', ['RESPONSEDATE' => "DATE_FORMAT(spm.shipment_test_report_date,'%Y-%m-%d')", '*'])
    ...
    public function fetchShipmentFormDetails($params, $dm)
    {
        ...
        if ($params['scheme_type'] == 'dts') {
            ...
            if (isset($participant) && !empty($participant)) {
                ...
                if ($dtsSchemeType == 'malawi' || (isset($displaySampleConditionFields) && $displaySampleConditionFields == 'yes')) {
                    ...
                    $section2['conditionOfPTSamplesSelect'] = [
                        [
                            'value' => 'good',
                            'show' => 'Good',
                ...
                foreach ($qcArray as $row) {
                    $qcResponseArr[] = ['value' => $row, 'show' => ucwords($row), 'selected' => (isset($shipment['qc_done']) && $shipment['qc_done'] == $row || (($shipment['qc_done'] == null || $shipment['qc_done'] == '') && $row == 'no')) ? 'selected' : ''];
                }
            ...
            $dtsPtNotTested['receivedPtPanelSelect'] = [
                ['value' => 'yes', 'show' => 'Yes', 'selected' => ($shipment['received_pt_panel'] == 'yes') ? 'selected' : ''],
                ['value' => 'no', 'show' => 'No', 'selected' => ($shipment['received_pt_panel'] == 'no') ? 'selected' : ''],
            ];
            ...
            foreach ($allTestKits as $testKitKey => $testkit) {
                if ($testkit['testkit_1'] == '1') {
                    ...
                    if ((isset($allowRepeatTests) && $allowRepeatTests)) {
                        ...
                        $teskitArray['kitNameDropdown']['Repeat Test-1']['data'][] = [
                            'value' => (string) $testkit['TESTKITNAMEID'],
                            'show' => $testkit['TESTKITNAME'],
            ...
            if ((isset($allowRepeatTests) && $allowRepeatTests)) {
                if (!$testThreeOptional) {
                    $teskitArray['kitText'] = ['Test-1', 'Test-2', 'Test-3', 'Repeat Test-1', 'Repeat Test-2', 'Repeat Test-3'];
                } else {
                    $teskitArray['kitText'] = ['Test-1', 'Test-2', 'Repeat Test-1', 'Repeat Test-2'];
                }
            } else {
                $teskitArray['kitText'] = ['Test-1', 'Test-2', 'Test-3'];
            }
            ...
            foreach ($allSamples as $sample) {
                ...
                $allSamplesResult['samples']['result1'][] = [
                ...
                $allSamplesResult['samples']['result2'][] = [
                ...
                if ((isset($allowRepeatTests) && $allowRepeatTests)) {
                    $allSamplesResult['samples']['repeatResult1'][] = [
                    ...
                    $allSamplesResult['samples']['repeatResult2'][] = [
                    ...
                    $allSamplesResult['samples']['repeatResult3'][] = [
                ...
                $allSamplesResult['samples']['finalResult'][] = [
                    ...
                    if ($row == 3) {
                        foreach ($dtsPossibleResults as $pr) {
                            if ($pr['scheme_sub_group'] == 'DTS_TEST') {
                                $possibleResults[] = ['value' => (string) $pr['id'], 'show' => $pr['response'], 'resultCode' => $pr['result_code'], 'selected' => ($sample['test_result_3'] == $pr['id']) ? 'selected' : ''];
                                if ((isset($allowRepeatTests) && $allowRepeatTests)) {
                                    $repeatPossibleResults[] = ['value' => (string) $pr['id'], 'show' => $pr['response'], 'resultCode' => $pr['result_code'], 'selected' => ($sample['repeat_test_result_3'] == $pr['id']) ? 'selected' : ''];
                                }
                        ...
                        foreach ($dtsPossibleResults as $pr) {
                            if ($pr['scheme_sub_group'] == 'DTS_TEST') {
                                $possibleResults[] = ['value' => (string) $pr['id'], 'show' => $pr['response'], 'resultCode' => $pr['result_code'], 'selected' => (($sample['test_result_1'] == $pr['id'] && $row == 1) || ($sample['test_result_2'] == $pr['id'] && $row == 2)) ? 'selected' : ''];
                                if ((isset($allowRepeatTests) && $allowRepeatTests)) {
                                    $repeatPossibleResults[] = ['value' => (string) $pr['id'], 'show' => $pr['response'], 'resultCode' => $pr['result_code'], 'selected' => (($sample['repeat_test_result_1'] == $pr['id'] && $row == 1) || ($sample['repeat_test_result_2'] == $pr['id'] && $row == 2)) ? 'selected' : ''];
                                }
                ...
                foreach ($dtsPossibleResults as $pr) {
                    if ($pr['scheme_sub_group'] == 'DTS_FINAL') {
                        $possibleFinalResults[] = ['value' => (string) $pr['id'], 'show' => $pr['response'], 'resultCode' => $pr['result_code'], 'selected' => ($sample['reported_result'] == $pr['id']) ? 'selected' : ''];
            ...
            foreach ($commentArray as $row) {
                $revieArr[] = ['value' => $row, 'show' => ucwords($row), 'selected' => (isset($shipment['supervisor_approval']) && $shipment['supervisor_approval'] == $row || (($shipment['supervisor_approval'] != null || $shipment['supervisor_approval'] != '') && $row == 'yes')) ? 'selected' : ''];
            }
        ...
        if ($params['scheme_type'] == 'vl') {
            ...
            $qc = [];
            foreach ($qcArray as $row) {
                $qcResponseArr[] = ['value' => (string) $row, 'show' => ucwords($row), 'selected' => (isset($shipment['qc_done']) && $shipment['qc_done'] == $row || (($shipment['qc_done'] == null || $shipment['qc_done'] == '') && $row == 'no')) ? 'selected' : ''];
            }
            $qc['qcRadio'] = $qcResponseArr;
            $qc['qcRadioSelected'] = (isset($shipment['qc_done']) && $shipment['qc_done'] == 'no' || $shipment['qc_done'] == null || $shipment['qc_done'] == '') ? 'no' : 'yes';
            $qc['qcDate'] = (isset($shipment['qc_date']) && $shipment['qc_date'] != '' && $shipment['qc_date'] != '0000:00:00' && $shipment['qc_date'] != null && $shipment['qc_date'] != '1969-12-31') ? date('d-M-Y', strtotime($shipment['qc_date'])) : '';
            ...
            $section3['data']['receivedPtPanelSelect'] = [
                ['value' => 'yes', 'show' => 'Yes', 'selected' => ($shipment['received_pt_panel'] == 'yes') ? 'selected' : ''],
                ['value' => 'no', 'show' => 'No', 'selected' => ($shipment['received_pt_panel'] == 'no') ? 'selected' : ''],
            ];
            ...
            foreach ($allSamples as $key => $sample) {
                ...
                foreach ($vlArray as $row) {
                    $vlResponseArr[] = ['value' => (string) $row, 'show' => ucwords($row), 'selected' => ($sample['is_tnd'] == $row || ($sample['is_tnd'] == '' && $row == 'no')) ? 'selected' : ''];
                }
            ...
            $revieArr = [];
            foreach ($commentArray as $row) {
                $revieArr[] = ['value' => (string) $row, 'show' => ucwords($row), 'selected' => (isset($shipment['supervisor_approval']) && $shipment['supervisor_approval'] == $row || (($shipment['supervisor_approval'] != null || $shipment['supervisor_approval'] != '') && $row == 'yes')) ? 'selected' : ''];
            }
            $reviewArray['supervisorReview'] = $revieArr;
            $reviewArray['supervisorReviewSelected'] = (isset($shipment['supervisor_approval']) && $shipment['supervisor_approval'] != '') ? $shipment['supervisor_approval'] : '';
            $reviewArray['approvalLabel'] = 'Supervisor Name';
        ...
        if ($params['scheme_type'] == 'eid') {
            ...
            foreach ($allSamples as $sample) {
                ...
                foreach ($eidPossibleResults as $pr) {
                    if ($pr['scheme_sub_group'] == 'EID_FINAL') {
                        $possibleEIDResults[] = ['value' => (string) $pr['id'], 'show' => $pr['response'], 'resultCode' => $pr['result_code'], 'selected' => ($sample['reported_result'] == $pr['id']) ? 'selected' : ''];
                    }
                }

                $allSamplesResult['resultsText'] = ['Control/Sample', 'Your-Results', 'HIV-CT/OD', 'IC/QS-Values'];
                $allSamplesResult['resultStatus'] = [true, true, true, true];
            ...
            $eid['Section3']['data']['receivedPtPanelSelect'] = [
                ['value' => 'yes', 'show' => 'Yes', 'selected' => ($shipment['received_pt_panel'] == 'yes') ? 'selected' : ''],
                ['value' => 'no', 'show' => 'No', 'selected' => ($shipment['received_pt_panel'] == 'no') ? 'selected' : ''],
            ];
            ...
            foreach ($allSamples as $sample) {
                ...
                foreach ($recencyPossibleResults as $pr) {
                    if ($pr['scheme_sub_group'] == 'RECENCY_FINAL') {
                        $possibleRecencyResults[] = ['value' => (string) $pr['id'], 'show' => $pr['response'], 'resultCode' => $pr['result_code'], 'selected' => ($sample['reported_result'] == $pr['id']) ? 'selected' : ''];
                    }
                }
                $possibleRecencyResults[] = ['value' => '', 'show' => '', 'resultCode' => '', 'selected' => ''];

                $ctlLineResults = [];
                $verifyLineResults = [];
                $longLineResults = [];
                $resultArray = ['present', 'absent'];
                foreach ($resultArray as $pr) {
                    $ctlLineResults[] = ['value' => (string) $pr, 'show' => ucwords($pr), 'selected' => ($sample['control_line'] == $pr) ? 'selected' : ''];
                }
                foreach ($resultArray as $pr) {
                    $verifyLineResults[] = ['value' => (string) $pr, 'show' => ucwords($pr), 'selected' => ($sample['diagnosis_line'] == $pr) ? 'selected' : ''];
                }
                foreach ($resultArray as $pr) {
                    $longLineResults[] = ['value' => (string) $pr, 'show' => ucwords($pr), 'selected' => ($sample['longterm_line'] == $pr) ? 'selected' : ''];
                }

                $recency['Section3']['data']['resultsText'] = ['Control/Sample', 'Control Line', 'Verification Line', 'Longterm Line', 'Your Result'];
                $recency['Section3']['data']['resultStatus'] = [true, true, true, true];
        ...
        if ($params['scheme_type'] == 'covid19') {
            ...
            foreach ($allTestTypes as $testtype) {
                if ($testtype['test_type_1'] == '1') {
                    ...
                    $testPlatformArray['testPlatformDropDown']['Test-1']['data'][] = [
                ...
                if ($testtype['test_type_2'] == '1') {
                    ...
                    $testPlatformArray['testPlatformDropDown']['Test-2']['data'][] = [
                        'value' => (string) $testtype['test_type_id'],
                        'show' => $testtype['test_type_name'],
                ...
                if ($testtype['test_type_3'] == '1') {
                    $testPlatformArray['testPlatformDropDown']['Test-3']['data'][] = [
                        'value' => (string) $testtype['test_type_id'],
                        'show' => $testtype['test_type_name'],
            ...
            $testPlatformArray['typeText'] = ['Test-1', 'Test-2', 'Test-3'];
            if (isset($allSamples) && count($allSamples) > 0) {
            ...
            foreach ($allSamples as $sample) {
                ...
                $allSamplesResult['resultsText'] = ['Result-1', 'Result-2', 'Result-3', 'Final-Result'];

                if (!$testThreeOptional) {
                    $allSamplesResult['resultStatus'] = [true, true, true, true];
                } elseif (!$testTwoOptional) {
                    $allSamplesResult['resultStatus'] = [true, false, false, true];
                } else {
                    $allSamplesResult['resultStatus'] = [true, true, false, true];
                }
                /* if ((isset($shipment['shipment_attributes']["screeningTest"]) && $shipment['shipment_attributes']["screeningTest"] == 'yes')) {
    ...
    public function fetchIndividualReportAPI($params)
    {
        ...
        $resultData = [];
        $sQuery = $this->getAdapter()->select()->from(['s' => 'shipment'], ['SHIP_YEAR' => 'year(s.shipment_date)', 's.scheme_type', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.status', 's.updated_on_admin'])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['scheme_name'])
            ->join(['spm' => 'shipment_participant_map'], 'spm.shipment_id=s.shipment_id', ['spm.map_id', 'spm.evaluation_status', 'spm.participant_id', 'RESPONSEDATE' => "DATE_FORMAT(spm.shipment_test_report_date,'%Y-%m-%d')", 'RESPONSE' => new Zend_Db_Expr("CASE substr(spm.evaluation_status,3,1) WHEN 1 THEN 'View' WHEN '9' THEN 'Enter Result' END"), 'REPORT' => new Zend_Db_Expr("CASE  WHEN spm.report_generated='yes' AND s.status='finalized' THEN 'Report' END")])
            ->join(['p' => 'participant'], 'p.participant_id=spm.participant_id', ['p.unique_identifier', 'p.first_name', 'p.last_name'])
            ->join(['pmm' => 'participant_manager_map'], 'pmm.participant_id=p.participant_id')
            ->where('pmm.dm_id=?', $aResult['dm_id'])
            ->where("s.status='shipped' OR s.status='evaluated'OR s.status='finalized'");
    ...
    public function fetchSummaryReportAPI($params)
    {
        ...
        $resultData = [];
        $sQuery = $this->getAdapter()->select()->from(['s' => 'shipment'], ['s.scheme_type', 's.shipment_date', 's.shipment_code', 's.status', 's.updated_on_admin'])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['scheme_name'])
            ->join(['spm' => 'shipment_participant_map'], 'spm.shipment_id=s.shipment_id', ['spm.map_id'])
    ...
    public function saveShipmentsFormDetailsByAPI($params)
    {
        ...
        if (isset($params['syncType']) && $params['syncType'] == 'group') {
            ...
            if ($responseStatus) {
                return ['status' => 'failure', 'data' => $returnResposne, 'profileInfo' => $dm['profileInfo']];
            } else {
                return ['status' => 'success', 'data' => $returnResposne, 'profileInfo' => $dm['profileInfo']];
            }
        }
        if (isset($params['syncType']) && $params['syncType'] == 'single') {
            ...
            // die($status);
            if ($status) {
                return ['status' => 'success', 'message' => 'Thank you for submitting your result. We have received it and the PT Results will be published on or after the due date.', 'profileInfo' => $dm['profileInfo']];
            } else {
                return ['status' => 'fail', 'message' => 'Please check your network connection and try again.', 'profileInfo' => $dm['profileInfo']];
            }
        }
    ...
    {
        $sQuery = $this->getAdapter()->select()
            ->from(['s' => 'shipment'], ['s.scheme_type', 's.shipment_date', 's.shipment_code', 's.lastdate_response', 's.shipment_id', 's.status', 's.response_switch', 'panelName' => new Zend_Db_Expr('shipment_attributes->>"$.panelName"')])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['scheme_name', 'is_user_configured'])
