...
class Application_Model_Tb
{
    ...
    public function generateTbExcelReport($shipmentId)
    {
        ...
        if ($result['scheme_type'] == 'tb') {
            ...
            $refQuery = $db->select()->from(array('refRes' => 'reference_result_tb'), array('refRes.sample_label', 'sample_id', 'refRes.sample_score'))
        ...
        //<-------- Second sheet start
        $reportHeadings = array('Participant Code', 'Participant Name', 'Region', 'Shipment Receipt Date', 'Testing Date', 'Assay Name', 'Assay Lot', 'Assay Expiration', 'MTBC', 'Rif Resistance', 'SPC', 'IS1081-IS6110', 'Probe D', 'Probe C', 'Probe E', 'Probe B', 'Probe A', 'rpoB1', 'rpoB2', 'rpoB3', 'rpoB4', 'Test Date', 'Tester Name', 'Error Code');

        $reportHeadings = $this->addTbSampleNameInArray($shipmentId, $reportHeadings);
        ...
        if (isset($shipmentResult) && count($shipmentResult) > 0) {
            ...
            foreach ($shipmentResult as $aRow) {
                ...
                $sheet->getCellByColumnAndRow($r++, $currentRow)->setValueExplicit($attributes['assay_name'], \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                $sheet->getCellByColumnAndRow($r++, $currentRow)->setValueExplicit($attributes['assay_lot_number'], \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
                $sheet->getCellByColumnAndRow($r++, $currentRow)->setValueExplicit($attributes['expiry_date'], \PhpOffice\PhpSpreadsheet\Cell\DataType::TYPE_STRING);
    ...
    public function addTbSampleNameInArray($shipmentId, $headings)
    {
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
