...
class Application_Model_Covid19
{
    ...
    public function generateCovid19ExcelReport($shipmentId)
    {
        ...
        if ($result['scheme_type'] == 'covid19') {
            ...
            $refQuery = $db->select()->from(['refRes' => 'reference_result_covid19'], ['refRes.sample_label', 'sample_id', 'refRes.sample_score'])
            ...
            if (count($refResult) > 0) {
                foreach ($refResult as $key => $refRes) {
                    $refCovid19Query = $db->select()->from(['refCovid19' => 'reference_covid19_test_type'], ['refCovid19.lot_no', 'refCovid19.expiry_date', 'refCovid19.result'])
                        ->joinLeft(['r' => 'r_possibleresult'], 'r.id=refCovid19.result', ['referenceTypeResult' => 'r.response'])
                        ->joinLeft(['tt' => 'r_test_type_covid19'], 'tt.test_type_id=refCovid19.test_type', ['testPlatformName' => 'tt.test_type_name'])
        ...
        $firstSheetHeading = ['Tab Name', 'Description'];
        $firstSheetColNo = 0;
        $firstSheetRow = 1;
        ...
        $sql = $db->select()->from(['s' => 'shipment'], ['s.shipment_id', 's.shipment_code', 's.number_of_samples'])
            ->join(['sp' => 'shipment_participant_map'], 'sp.shipment_id=s.shipment_id', ['sp.map_id', 'sp.participant_id', 'sp.attributes', 'sp.shipment_test_date', 'sp.shipment_receipt_date', 'sp.shipment_test_report_date', 'sp.supervisor_approval', 'sp.participant_supervisor', 'sp.shipment_score', 'sp.documentation_score', 'sp.user_comment'])
        ...
        if (isset($shipmentResult) && !empty($shipmentResult)) {
            ...
            foreach ($shipmentResult as $key => $aRow) {
                if ($result['scheme_type'] == 'covid19') {
                    $resQuery = $db->select()->from(['rrcovid19' => 'response_result_covid19'])
                        ->joinLeft(['tt1' => 'r_test_type_covid19'], 'tt1.test_type_id=rrcovid19.test_type_1', ['testPlatformName1' => 'tt1.test_type_name'])
                        ->joinLeft(['tt2' => 'r_test_type_covid19'], 'tt2.test_type_id=rrcovid19.test_type_2', ['testPlatformName2' => 'tt2.test_type_name'])
                        ->joinLeft(['tt3' => 'r_test_type_covid19'], 'tt3.test_type_id=rrcovid19.test_type_3', ['test~PlatformName3' => 'tt3.test_type_name'])
                        ->joinLeft(['r' => 'r_possibleresult'], 'r.id=rrcovid19.test_result_1', ['testResult1' => 'r.response'])
                        ->joinLeft(['rp' => 'r_possibleresult'], 'rp.id=rrcovid19.test_result_2', ['testResult2' => 'rp.response'])
                        ->joinLeft(['rpr' => 'r_possibleresult'], 'rpr.id=rrcovid19.test_result_3', ['testResult3' => 'rpr.response'])
                        ->joinLeft(['fr' => 'r_possibleresult'], 'fr.id=rrcovid19.reported_result', ['finalResult' => 'fr.response'])
    ...
    {
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $query = $db->select()->from('reference_result_covid19', ['sample_label'])
