...
final class Application_Model_Dts
{
    ...
    public function evaluate($shipmentResult, $shipmentId, $reEvaluate = false)
    {
        ...
        // } else {
        $this->db->update('shipment', ['max_score' => $maxScore, 'average_score' => $averageScore, 'status' => 'evaluated'], $shipmentWhere);
        // }
    ...
    private function evaluateSingleShipment(array $shipment, array $results, array $context)
    {
        ...
        if ($shipment['is_excluded'] != 'yes' && $shipment['is_pt_test_not_performed'] != 'yes') {
            ...
            foreach ($correctiveActionList as $ca) {
                $this->db->insert('dts_shipment_corrective_action_map', ['shipment_map_id' => $shipment['map_id'], 'corrective_action_id' => $ca]);
            }
        }
    ...
    private function getShipmentResult($shipmentId)
    {
        ...
        $sql = $this->db->select()->from(['s' => 'shipment'], ['s.shipment_id', 's.shipment_code', 's.number_of_samples', 's.number_of_controls', 'shipment_attributes'])
    ...
    private function getParticipantResponse($mapId)
    {
        ...
        $responseQuery = $this->db->select()->from(['rrdts' => 'response_result_dts'])
            ->joinLeft(['tk1' => 'r_testkitnames'], 'tk1.TestKitName_ID=rrdts.test_kit_name_1', ['testKitName1' => 'tk1.TestKit_Name'])
            ->joinLeft(['tk2' => 'r_testkitnames'], 'tk2.TestKitName_ID=rrdts.test_kit_name_2', ['testKitName2' => 'tk2.TestKit_Name'])
            ->joinLeft(['tk3' => 'r_testkitnames'], 'tk3.TestKitName_ID=rrdts.test_kit_name_3', ['testKitName3' => 'tk3.TestKit_Name'])
            ->joinLeft(['r' => 'r_possibleresult'], 'r.id=rrdts.test_result_1', ['testResult1' => 'r.response'])
            ->joinLeft(['rp' => 'r_possibleresult'], 'rp.id=rrdts.test_result_2', ['testResult2' => 'rp.response'])
            ->joinLeft(['rpr' => 'r_possibleresult'], 'rpr.id=rrdts.test_result_3', ['testResult3' => 'rpr.response'])
            ->joinLeft(['rpr1' => 'r_possibleresult'], 'rpr1.id=rrdts.repeat_test_result_1', ['repeatTestResult1' => 'rpr1.response'])
            ->joinLeft(['rpr2' => 'r_possibleresult'], 'rpr2.id=rrdts.repeat_test_result_2', ['repeatTestResult2' => 'rpr2.response'])
            ->joinLeft(['rpr3' => 'r_possibleresult'], 'rpr3.id=rrdts.repeat_test_result_3', ['repeatTestResult3' => 'rpr3.response'])
            ->joinLeft(['fr' => 'r_possibleresult'], 'fr.id=rrdts.reported_result', ['finalResult' => 'fr.response'])
            ->joinLeft(['rtrifr' => 'r_possibleresult'], 'rtrifr.id=rrdts.dts_rtri_reported_result', ['rtrifinalResult' => 'rtrifr.response'])
    ...
    private function getShipmentReferenceResults($shipmentId)
    {
        ...
        $referenceResultsQuery = $this->db->select()->from(['refRes' => 'reference_result_dts'], ['refRes.sample_label', 'sample_id', 'refRes.sample_score', 'refRes.dts_rtri_reference_result'])
            ->joinLeft(['r' => 'r_possibleresult'], 'r.id=refRes.reference_result', ['referenceResult' => 'r.response'])
            ->joinLeft(['rtri' => 'r_possibleresult'], 'rtri.id=refRes.dts_rtri_reference_result', ['rtriReferenceResult' => 'rtri.response'])
            ->where('refRes.shipment_id = ?', $shipmentId);
        $refResult = $this->db->fetchAll($referenceResultsQuery);
        if (!empty($refResult)) {
            foreach ($refResult as $key => $refRes) {
                $refDtsQuery = $this->db->select()->from(['refDts' => 'reference_dts_rapid_hiv'], ['refDts.lot_no', 'refDts.expiry_date', 'refDts.result'])
                    ->joinLeft(['r' => 'r_possibleresult'], 'r.id=refDts.result', ['referenceKitResult' => 'r.response'])
                    ->joinLeft(['tk' => 'r_testkitnames'], 'tk.TestKitName_ID=refDts.testkit', ['testKitName' => 'tk.TestKit_Name'])
    ...
    private function algoVietnam(
        ...
        if ($isScreening) {
            ...
            if ($final === 'P') {
                $this->vietnamFeedback($out, $sampleLabel, $SCREEN_NO_POS, true);
                return;
            }
        ...
        if ($final === 'N') {
            $this->vietnamFeedback($out, $sampleLabel, $REVIEW_SAMPLE, true);
            return;
        }
