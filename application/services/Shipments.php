...
class Application_Service_Shipments
{
    ...
    /**
     * Stamp shipment_participant_map.started_at the first time a participant opens the
     * response form. This is the "started the test" moment the late-submission check
     * compares against - the participant may open the form before the deadline and only
     * finish (confirm) after it.
     *
     * Set once and never overwritten. No-op for admin-side access, for closed/finalized
     * shipments (a start recorded after the shipment shut can never make a submission
     * "late") and for view-only data managers - all covered by isShipmentEditable().
     */
    public function recordResponseStart($shipmentId, $participantId): void
    {
        $shipmentId = (int) $shipmentId;
        $participantId = (int) $participantId;
        if ($shipmentId <= 0 || $participantId <= 0) {
            return;
        }
        if (!$this->isShipmentEditable($shipmentId, $participantId)) {
            return;
        }
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $db->update(
            'shipment_participant_map',
            ['started_at' => new Zend_Db_Expr('now()')],
            $db->quoteInto('shipment_id = ?', $shipmentId)
                . ' AND ' . $db->quoteInto('participant_id = ?', $participantId)
                . ' AND started_at IS NULL'
        );
    }

    /**
     * True when a participant may still save a response even though the shipment has
     * auto-closed at its deadline - a grace window for "started before the deadline,
     * finishing just after it".
     *
     * All must hold: the late_submission_grace_minutes config is > 0; the caller is not a
     * view-only data manager; the shipment is not finalized or cancelled; the participant's
     * started_at is on/before the deadline; and now is strictly after the deadline but
     * within the grace window. The strict "after the deadline" lower bound means a shipment
     * a coordinator closed early is never reopened by this.
     */
    public function isWithinLateSubmissionGrace($shipmentId, $participantId): bool
    {
        $graceMinutes = (int) (Application_Service_Common::getConfig('late_submission_grace_minutes') ?: 0);
        if ($graceMinutes <= 0) {
            return false;
        }

        $dm = new Zend_Session_Namespace('datamanagers');
        if (($dm->view_only_access ?? '') === 'yes') {
            return false;
        }

        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $shipment = $db->fetchRow(
            $db->select()->from('shipment', ['status', 'cancelled_at', 'response_deadline'])
                ->where('shipment_id = ?', (int) $shipmentId)
        );
        if (!$shipment || $shipment['status'] === 'finalized' || !empty($shipment['cancelled_at'])) {
            return false;
        }

        $startedAt = $db->fetchOne(
            $db->select()->from('shipment_participant_map', 'started_at')
                ->where('shipment_id = ?', (int) $shipmentId)
                ->where('participant_id = ?', (int) $participantId)
        );
        if (empty($startedAt) || strncmp((string) $startedAt, '0000', 4) === 0) {
            return false;
        }

        $cutoff = Pt_Commons_DateUtility::shipmentCutoff($shipment['response_deadline']);
        $startInstant = Pt_Commons_DateUtility::responseInstant($startedAt);
        if (!$cutoff || !$startInstant || $startInstant > $cutoff) {
            return false;
        }

        $now = new DateTimeImmutable('now', new DateTimeZone('UTC'));
        $graceEnd = $cutoff->modify('+' . $graceMinutes . ' minutes');
        return $now > $cutoff && $now <= $graceEnd;
    }

    /**
     * Late-submission capture (participant side).
     *
     * started_at is stamped when the participant opens the response form (see
     * recordResponseStart()). When that start was on or before the shipment deadline but
     * the response is being saved after it, the result is still stored in full but held as
     * response_status = 'late_submitted' with late_submit_status = 1, until an admin
     * approves it (approval flips the status back to 'responded' while keeping the flag = 1
     * as history). Deadline vs. instant comparison uses the same helpers the evaluator uses
     * so both code paths agree. Admin edits (reqAccessFrom = 'admin') are exempt. If no
     * start was ever recorded, the row is left as an ordinary response.
     *
     * @param  string $responseStatus status already computed by responseStatusForSubmission()
    ...
    private static function lateSubmissionCapture(array $params, string $responseStatus): array
    {
        ...
        $row = $mapId > 0
            ? $db->fetchRow(
                $db->select()
                    ->from('shipment_participant_map', ['started_at'])
                    ->columns(['db_now' => new Zend_Db_Expr('NOW()')])
                    ->where('map_id = ?', $mapId)
            )
            : null;
        ...
        if (empty($startedAt) || strncmp((string) $startedAt, '0000', 4) === 0) {
            // No recorded start (participant reached the POST without opening the form) -
            // treat as an ordinary response.
            return ['responseStatus' => $responseStatus, 'columns' => $columns];
        }

        $deadline = !empty($params['shipmentId'])
            ? $db->fetchOne($db->select()->from('shipment', 'response_deadline')->where('shipment_id = ?', $params['shipmentId']))
            : null;
        if (empty($deadline)) {
            $deadline = $params['hdLastDate'] ?? null;
        }

        $cutoff = Pt_Commons_DateUtility::shipmentCutoff($deadline);
        $startInstant = Pt_Commons_DateUtility::responseInstant($startedAt);
        $submitInstant = Pt_Commons_DateUtility::responseInstant($dbNow);

        $isLateSubmit = $cutoff && $startInstant && $submitInstant
            && $startInstant <= $cutoff && $submitInstant > $cutoff;

        if ($isLateSubmit && ($params['isPtTestNotPerformed'] ?? '') !== 'yes') {
            $responseStatus = 'late_submitted';
            $columns['late_submit_status'] = 1;
        }
    ...
    public function updateEidResults($params)
    {
        ...
        try {
            ...
            if (isset($params['sampleRehydrationDate']) && trim($params['sampleRehydrationDate']) != '') {
                $params['sampleRehydrationDate'] = Pt_Commons_DateUtility::isoDateFormat($params['sampleRehydrationDate']);
            } else {
                $params['sampleRehydrationDate'] = '';
            }
            if (isset($params['extractionAssayExpiryDate']) && trim($params['extractionAssayExpiryDate']) != '') {
                $params['extractionAssayExpiryDate'] = Pt_Commons_DateUtility::isoDateFormat($params['extractionAssayExpiryDate']);
            } else {
                $params['extractionAssayExpiryDate'] = '';
            }
            if (isset($params['detectionAssayExpiryDate']) && trim($params['detectionAssayExpiryDate']) != '') {
                $params['detectionAssayExpiryDate'] = Pt_Commons_DateUtility::isoDateFormat($params['detectionAssayExpiryDate']);
            } else {
    ...
    public function updateRecencyResults($params)
    {
        ...
        $mandatoryFields = ['receiptDate', 'testDate', 'sampleRehydrationDate'];
        ...
        try {
            ...
            if (isset($params['recencyAssayExpiryDate']) && trim($params['recencyAssayExpiryDate']) != '') {
                $params['recencyAssayExpiryDate'] = Pt_Commons_DateUtility::isoDateFormat($params['recencyAssayExpiryDate']);
            } else {
    ...
    public function updateDtsResults($params)
    {
        ...
        try {
            ...
            foreach ($params['avilableTestKit'] as $kit) {
                ...
                $db->insert('participant_testkit_map', [
                    'participant_id' => $params['participantId'],
    ...
    public function updateTbResults($params)
    {
        ...
        try {
            ...
            $attributes = [
                ...
                'expiry_date' => (isset($params['expiryDate']) && !empty($params['expiryDate'])) ? Pt_Commons_DateUtility::isoDateFormat($params['expiryDate']) : '',
            ...
            if (isset($params['serialNo']) && sizeof($params['serialNo']) > 0) {
                ...
                foreach ($params['serialNo'] as $key => $tbInstrument) {
                    $instrumentData = [
                        ...
                        'instrument_installed_on' => Pt_Commons_DateUtility::isoDateFormat($params['installedOn'][$key]),
                        'instrument_last_calibrated_on' => Pt_Commons_DateUtility::isoDateFormat($params['lastCalibrated'][$key]),
                    ];
    ...
    public function updateVlResults($params)
    {
        ...
        try {
            ...
            if (isset($params['assayExpirationDate']) && trim($params['assayExpirationDate']) != '') {
                $params['assayExpirationDate'] = Pt_Commons_DateUtility::isoDateFormat($params['assayExpirationDate']);
            }
                    ...
                    for ($e = 0; $e < $eiaSize; $e++) {
                        if (isset($params['rtype'][$i + 1]['type'][$e]) && trim($params['rtype'][$i + 1]['type'][$e]) != '') {
                            ...
                            if (trim($params['rtype'][$i + 1]['expiry'][$e]) != '') {
                                $expDate = Pt_Commons_DateUtility::isoDateFormat($params['rtype'][$i + 1]['expiry'][$e]);
                            }
    ...
    public function getShipmentForEdit($sid)
    {
        ...
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $query = $db->select()->from(['s' => 'shipment'])
            ->join(['scheme' => 'scheme_list'], 'scheme.scheme_id=s.scheme_type', ['scheme_name', 'is_user_configured'])
            ->join(['d' => 'distributions'], 'd.distribution_id = s.distribution_id', ['distribution_code', 'distribution_date'])
        ...
        if ($shipment['scheme_type'] == 'dts') {
            ...
            $reference = $db->fetchAll($db->select()->from(['s' => 'shipment'])
                ->join(['ref' => 'reference_result_dbs'], 'ref.shipment_id=s.shipment_id')
                ->where('s.shipment_id = ?', $sid));
            $schemeService = new Application_Service_Schemes();
            ...
            $reference = $db->fetchAll($db->select()->from(['s' => 'shipment'])
                ->join(['ref' => 'reference_result_eid'], 'ref.shipment_id=s.shipment_id')
                ->where('s.shipment_id = ?', $sid));
            $schemeService = new Application_Service_Schemes();
            $possibleResults = $schemeService->getPossibleResults('eid');
        } elseif ($shipment['scheme_type'] == 'vl') {
            $reference = $db->fetchAll($db->select()->from(['s' => 'shipment'])
                ->join(['ref' => 'reference_result_vl'], 'ref.shipment_id=s.shipment_id')
                ->where('s.shipment_id = ?', $sid));
            $possibleResults = '';
            ...
            $reference = $db->fetchAll($db->select()->from(['s' => 'shipment'])
                ->join(['ref' => 'reference_result_tb'], 'ref.shipment_id=s.shipment_id')
                ->where('s.shipment_id = ?', $sid));
            $reference = Pt_Commons_MiscUtility::sortBySampleLabelNatural($reference);
            $possibleResults = '';
            ...
            $reference = $db->fetchAll($db->select()->from(['s' => 'shipment'])
                ->join(['ref' => 'reference_result_recency'], 'ref.shipment_id=s.shipment_id')
                ->where('s.shipment_id = ?', $sid));
            $schemeService = new Application_Service_Schemes();
            $possibleResults = $schemeService->getPossibleResults('recency');
        } elseif ($shipment['scheme_type'] == 'covid19') {
            $reference = $db->fetchAll($db->select()->from(['s' => 'shipment'])
                ->join(['ref' => 'reference_result_covid19'], 'ref.shipment_id=s.shipment_id')
                ->where('s.shipment_id = ?', $sid));
            $schemeService = new Application_Service_Schemes();
            ...
            $reference = $db->fetchAll($db->select()->from(['s' => 'shipment'])
                ->join(['ref' => 'reference_result_generic_test'], 'ref.shipment_id=s.shipment_id')
                ->where('s.shipment_id = ?', $sid));
            $possibleResults = '';
        } else {
            return false;
    ...
    public function getParticipantCountBasedOnScheme()
    {
        ...
        $sQuery = $db->select()->from(['s' => 'shipment'], [])
            ->join(['sp' => 'shipment_participant_map'], 'sp.shipment_id=s.shipment_id', ['participantCount' => new Zend_Db_Expr('count(sp.participant_id)')])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['SCHEME' => 'sl.scheme_id'])
    ...
    public function sendShipmentMailAlertToParticipants($sid)
    {
        ...
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $return = 0;
        $sQuery = $db->select()->from(['sp' => 'shipment_participant_map'], ['sp.participant_id', 'sp.map_id', 'sp.new_shipment_mail_count'])
            ->join(['s' => 'shipment'], 's.shipment_id=sp.shipment_id', ['s.shipment_code', 's.shipment_code'])
            ->join(['d' => 'distributions'], 'd.distribution_id = s.distribution_id', ['distribution_code', 'distribution_date'])
            ->join(['p' => 'participant'], 'p.participant_id=sp.participant_id', ['p.email', 'participantName' => new Zend_Db_Expr(Application_Model_DbTable_Participants::participantNameGroupConcatExpr('p'))])
            ->join(['sl' => 'scheme_list'], 'sl.scheme_id=s.scheme_type', ['SCHEME' => 'sl.scheme_name'])
    ...
    public function getSchemeTypeShipmentDetailsInAPIV2($params)
    {
        $shipmentDb = new Application_Model_DbTable_Shipments();
        return $shipmentDb->fetchSchemeTypeShipmentDetailsInAPIV2($params);
    }
    ...
    public function getShipmentListBasedOnParticipant($params)
    {
        ...
        // }
        // }
        return ['result' => $response, 'monthRange' => $params['shipmentType']];
    }
        ...
        foreach ($rResult as $aRow) {
            if ($aRow['is_excluded'] != 'yes' && isset($aRow['REPORT']) && $aRow['REPORT'] != '') {
                ...
                if (file_exists($invididualFilePath) && file_exists($summaryFilePath)) {
                    ...
                    $search = ['##NAME##', '##SHIPCODE##', '##SHIPTYPE##', '##IND_REPORT_LINK##', '##SUM_REPORT_LINK##'];
                    $replace = [$aRow['participantName'], $aRow['shipment_code'], $aRow['scheme_name'], $indLink, $sumLink];
                    $content = $newShipmentMailContent['mail_content'];
                    $message = str_replace($search, $replace, $content);
                    $subject = str_replace($search, $replace, $newShipmentMailContent['mail_subject']);
                    $fromEmail = $newShipmentMailContent['mail_from'];
    ...
    {
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $sql = $db->select()->from(['d' => 'distributions', ['distribution_code', 'distribution_date']])
            ->join(['s' => 'shipment'], 'd.distribution_id=s.distribution_id', ['shipment_id', 'shipment_code', 'status', 'number_of_samples'])
            ->where('s.distribution_id = ?', $distributionId)
            ->group('s.shipment_id');
        return $db->fetchAll($sql);
    }
    ...
    {
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $sql = $db->select()->distinct()->from(['c' => 'r_dts_corrective_actions'])
    ...
    {
        $headings = ['PARTICIPANT ID', 'PARTICIPANT NAME', 'INSTITUDE / DEPARTMENT', 'PT SURVEY CODE', 'PT SURVEY DATE', 'SHIPMENT CODE', 'SHIPMENT DUE DATE', 'RESULT DUE DATE', 'SCHEME', 'NO OF SAMPLES', 'FINAL RESULTS', 'SCORE', 'CORRECTIVE ACTIONS', 'ACTION TAKEN', 'ACTION DATE'];
        try {
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = Pt_Commons_DateUtility::humanReadableDateFormat($aRow['action_date']);
                $output[] = $row;
            }
            ...
            foreach ($output as $rowNo => $rowData) {
                ...
                foreach ($rowData as $field => $value) {
                    ...
                    $sheet->getCell(Coordinate::stringFromColumnIndex($colNo + 1) . $rowNo + 4)
                        ->setValueExplicit(html_entity_decode($value, ENT_QUOTES, 'UTF-8'));
                    if ($colNo == (sizeof($headings) - 1)) {
                        $sheet->getColumnDimensionByColumn($colNo)->setWidth(150);
                        $sheet->getStyle(Coordinate::stringFromColumnIndex($colNo + 1) . $rowNo + 4, null, null)->getAlignment()->setWrapText(true);
                    }
                    $colNo++;
                }
            }
    ...
    {
        $headings = ['CORRECTIVE ACTIONS', 'DESCRIPTION', 'ACTIONS TAKEN', 'ACTIONS DATE'];
        try {
            $excel = new Spreadsheet();
            ...
            $sheet->getStyle(Coordinate::stringFromColumnIndex(1) . 1)->getFont()->setBold(true);
            $dataRow = 3;
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(4) . 2)
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(4) . 3)
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(4) . 4)
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(4) . 5)
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(4) . 6)
                ->setValueExplicit(html_entity_decode('Score', ENT_QUOTES, 'UTF-8'));
            foreach (range(2, 6) as $no) {
                $sheet->getStyle(Coordinate::stringFromColumnIndex(1) . $no)->getFont()->setBold(true);
                $sheet->getStyle(Coordinate::stringFromColumnIndex(4) . $no)->getFont()->setBold(true);
            }
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(5) . 2)
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(5) . 3)
            ...
            $sheet->getCell(Coordinate::stringFromColumnIndex(2) . 4)
                ->setValueExplicit(html_entity_decode(Pt_Commons_DateUtility::humanReadableDateFormat($result['distribution_date']), ENT_QUOTES, 'UTF-8'));
            $sheet->getCell(Coordinate::stringFromColumnIndex(5) . 4)
            ...
            foreach ($rResult as $aRow) {
                ...
                $row[] = Pt_Commons_DateUtility::humanReadableDateFormat($aRow['action_date']) ?? null;
            ...
            foreach ($output as $rowNo => $rowData) {
                ...
                foreach ($rowData as $field => $value) {
                    ...
                    $sheet->getCell(
                        Coordinate::stringFromColumnIndex($colNo + 1) . $rowNo + ($dataRow + 1)
                    )->setValueExplicit(html_entity_decode($value, ENT_QUOTES, 'UTF-8'));
                    if ($colNo == (sizeof($headings) - 1)) {
                        $sheet->getColumnDimensionByColumn($colNo)->setWidth(150);
                        $sheet->getStyle(Coordinate::stringFromColumnIndex($colNo + 1) . $rowNo + 4, null, null)->getAlignment()->setWrapText(true);
    ...
    {
        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $sQuery = $db->select()->from(['s' => 'shipment'], ['shipment_attributes', 'scheme_type'])
            ->where('s.shipment_id = ?', $sid);
