...
class Application_Service_Reports {
	...
	public function getShipmentParticipant($shipmentId){
		...
		$sql = $db->select()->from(array('s' => 'shipment'), array('s.shipment_id', 's.shipment_code','s.number_of_samples'))
				->join(array('sp' => 'shipment_participant_map'), 'sp.shipment_id=s.shipment_id', array('sp.map_id', 'sp.participant_id','attributes','sp.shipment_test_date','sp.shipment_receipt_date','sp.shipment_test_report_date','sp.user_comment'))
		...
		//<-------- Second sheet start
		$reportHeadings = array('Facility Code','Facility Name','Region','Shipment Receipt Date','Sample Rehydration Date','Testing Date','Test#1 Name','Kit Lot #','Exp Date');
		...
		$sheetThree->getDefaultColumnDimension()->setWidth(15);
		...
		foreach ($reportHeadings as $field => $value) {
			...
			if($colNo>=$finalResColoumn){
				if($c<=$result['number_of_samples']){
					...
					$cellName = $sheet->getCellByColumnAndRow($colNo,3)->getColumn();
					$sheet->getStyle($cellName.'3')->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setARGB('#808080');
		...
		if(isset($shipmentResult) && count($shipmentResult) > 0){
			...
			foreach ($shipmentResult as $aRow) {
				...
				if(isset($aRow['shipment_test_date']) && trim($aRow['shipment_test_date'])!=""){
					$aRow['shipment_test_date']=Pt_Commons_General::excelDateFormat($aRow['shipment_test_date']);
				}
				...
				$sheet->getCellByColumnAndRow($r++, $currentRow)->setValueExplicit($aRow['shipment_test_date'], PHPExcel_Cell_DataType::TYPE_STRING);
