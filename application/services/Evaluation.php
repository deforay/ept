	...
	public function getIndividualReportsDataForPDF($shipmentId, $sLimit = null, $sOffset = null)
	{
		...
		foreach ($sRes as $res) {
			...
			if ($res['scheme_type'] == 'dbs') {
				...
				$sQuery = $db->select()->from(array('resdts' => 'response_result_dts'), array('resdts.shipment_map_id', 'resdts.sample_id', 'resdts.reported_result', 'calculated_score', 'test_kit_name_1', 'lot_no_1', 'exp_date_1', 'test_kit_name_2', 'lot_no_2', 'exp_date_2', 'test_kit_name_3', 'lot_no_3', 'exp_date_3', 'test_result_1', 'test_result_2', 'test_result_3', 'repeat_test_result_1', 'repeat_test_result_2', 'repeat_test_result_3', 'is_this_retest', 'syphilis_result', 'syphilis_final', 'dts_rtri_control_line', 'dts_rtri_diagnosis_line', 'dts_rtri_longterm_line', 'dts_rtri_reported_result', 'dts_rtri_is_editable'))
					...
					->join(array('refdts' => 'reference_result_dts'), 'refdts.shipment_id=sp.shipment_id and refdts.sample_id=resdts.sample_id', array('refdts.reference_result', 'refdts.sample_label', 'refdts.mandatory', 'refdts.sample_score', 'refdts.control', 'dts_rtri_reference_result'))
	...
	public function getEvaluateReportsInPdf($shipmentId, $sLimit, $sOffset)
	{
		...
		foreach ($shipmentResult as $res) {
			...
			foreach ($tbResults as $tbResult) {
				if (
					in_array($tbResult['mtb_detected'], array("notDetected", "noResult", "invalid", "error")) &&
				...
				if (isset($tbResultsConsensusMtbRif[$tbResult['sample_id']])) {
					...
					if (
						isset($tbResultsConsensusMtbRif[$tbResult['sample_id']]['rif_resistance']) &&
				...
				if (isset($tbResultsConsensusUltra[$tbResult['sample_id']])) {
					...
					if (
						isset($tbResultsConsensusUltra[$tbResult['sample_id']]['rif_resistance']) &&
			...
			if ($qcDoneOnTime) {
				$secondsSinceQcDoneToLastTest = strtotime(Pt_Commons_General::dateFormat($lastTestDate)) - strtotime(Pt_Commons_General::dateFormat($shipmentResult[$i]['qc_date']));
			...
			$shipmentResult[$i]['documentation_score'] = $scoringService->calculateTbDocumentationScore(
				$res['shipment_date'],
				$attributes['expiry_date'],
				$res['shipment_receipt_date'],
				$res['supervisor_approval'],
				$res['participant_supervisor'],
				$res['lastdate_response']
			);
			$shipmentResult[$i]['submission_score_status'] = $scoringService->calculateSubmissionPassStatus(
				$shipmentScore,
				$shipmentResult[$i]['documentation_score'],
				$maxShipmentScore,
				$sampleStatuses
			);
